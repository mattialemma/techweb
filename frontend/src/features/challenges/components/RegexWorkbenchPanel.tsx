...
// Scopo: Gestisce UI del modulo tentativo e stati risolta/autore per una sfida.
// Livello: Componente UI funzionalita
// Esporta: RegexWorkbenchPanel
// Dipende da: controlli modulo condivisi, limiti validazione, tipo Attempt
...
function AttemptForm({
  ...
  "fieldError" | "isSubmitting" | "lastAttempt" | "onRegexChange" | "onSubmit" | "proposedRegex" | "submitError"
>) {
  return (
    <form className="mt-8 space-y-6" onSubmit={onSubmit}>
      {submitError ? <InlineMessage tone="error">{submitError}</InlineMessage> : null}
      {lastAttempt ? (
        <InlineMessage tone="info">{`Feedback: ${formatAttemptSummary(lastAttempt)}.`}</InlineMessage>
      ) : null}
...
export function RegexWorkbenchPanel({
  ...
  const solvedMessage = `Hai gia risolto questa sfida al tentativo ${solvedAttemptNumber}.`;
  ...
  return (
    <Panel padding="lg" className="mx-auto w-full max-w-[560px]">
      ...
      {isAuthor ? (
        ...
        <div className="mt-6">
          <InlineMessage tone="success">{solvedMessage}</InlineMessage>
        </div>
      ) : (
        <AttemptForm
